...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleSpringAccessDenied(
        ...
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ApiResponse.error(messageUtil.get("error.auth.access.denied")));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleInsufficientAuth(InsufficientAuthenticationException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(ApiResponse.error(messageUtil.get("error.auth.unauthorized")));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleNoAgent(NoAgentAvailableException ex) {
        log.warn("No agent available: {}", ex.getMessage());
    ...
    public ResponseEntity<ApiResponse<Void>> handleValidation(MethodArgumentNotValidException ex) {
        // Extract all error messages and join them with a newline or bullet points
    ...
    public ResponseEntity<ApiResponse<Void>> handleGeneral(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponse.error(messageUtil.get("error.internal.server")));
    }
}